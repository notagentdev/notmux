...
/// Drag preview: a small themed chip showing the dragged entry's name.
...
pub struct FileExplorer {
    ...
    /// Per-directory rollup (Added/Modified), precomputed in `apply_status`.
    dir_rollups: HashMap<String, FileStatus>,
...
impl FileExplorer {
    ...
    /// Whether the inline rename/new-file/new-folder input currently has
    /// keyboard focus (the host must not steal focus while the user types).
    pub fn input_focused(&self, window: &Window, cx: &App) -> bool {
        self.active_input
            .as_ref()
    ...
    /// Precomputed in `apply_status`; render only does a map lookup.
    ...
    pub fn start_rename(&mut self, target: PathBuf, cx: &mut Context<Self>) {
        ...
        self.active_input =
            Some(ActiveInput { mode: InputMode::Rename { target }, input, needs_focus: true });
        cx.notify();
    }
...
fn compute_dir_rollups(
    ...
    let mut rollups: HashMap<String, FileStatus> = HashMap::new();
    for (path, status) in status_by_relpath {
        let class = match status {
            FileStatus::Added | FileStatus::Untracked => FileStatus::Added,
        ...
        while let Some(part) = parts.next() {
            ...
            dir.push_str(part);
            let entry = rollups.entry(dir.clone()).or_insert(class);
            if class == FileStatus::Modified {
                *entry = FileStatus::Modified;
            }
        }
    }
    rollups
...
fn collect_rows(
    ...
    let Some(entries) = loaded.get(&dir) else {
        ...
        if ghost_parent == Some(dir.as_path()) {
            out.push(Row { entry: None, level, is_expanded: false, ghost_is_folder });
        }
        return;
    };
...
impl FileExplorer {
    fn render_row(
        ...
        div()
            ...
            // reference the file there.
            .on_drag(
                FileDrag { path: abs_path.clone(), name: entry.name.clone() },
                move |drag, _position, _window, cx| {
                    cx.new(|_| FileDragView { name: drag.name.clone() })
                },
            )
