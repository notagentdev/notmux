...
impl Render for SettingsPanel {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        div()
            ...
            .when(
                self.project_dropdown_open && self.project_button_bounds.is_some(),
                |panel| {
                    panel.child(dropdown_anchored_below(
                        self.project_button_bounds
            ...
            .when(
                self.font_dropdown_open && self.font_button_bounds.is_some(),
                |panel| {
                    let current = settings_entity(cx).read(cx).settings.font_family.clone();
                    panel.child(dropdown_anchored_below(
                        self.font_button_bounds
                            .expect("guarded by is_some() in when()"),
            ...
            .when(
                self.shell_dropdown_open && self.shell_button_bounds.is_some(),
                |panel| {
                    let current = settings_entity(cx).read(cx).settings.default_shell.clone();
                    panel.child(dropdown_anchored_below(
                        self.shell_button_bounds
                            .expect("guarded by is_some() in when()"),
            ...
            .when(
                self.session_backend_dropdown_open && self.session_backend_button_bounds.is_some(),
                |panel| {
                    let current = settings_entity(cx).read(cx).settings.session_backend;
                    panel.child(dropdown_anchored_below(
                        self.session_backend_button_bounds
                            .expect("guarded by is_some() in when()"),
