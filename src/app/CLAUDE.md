...
The `NotMux` entity is the central coordinator that owns the top-level GPUI entities (RootView, Workspace, RequestBroker, PtyManager) and routes events between them.

## Files

| File | Purpose |
|------|---------|
| `mod.rs` | `NotMux` struct — owns all top-level entities. Runs the PTY event loop (batched `async_channel` processing). Sets up workspace auto-save observer. |
