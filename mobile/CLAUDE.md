...
Remote terminal client for Android/iOS. Connects to the NotMux desktop server via REST + WebSocket. Uses `alacritty_terminal` in Rust for ANSI processing — identical terminal emulation as the desktop app.
...
└──────────┼──────────────────────────┼─────────────────────────┘
           ...
           ▼                          ▼
┌──────────────────────────────────────────────────────────────┐
│  NotMux Desktop (remote server on src/remote/)                │
...
├── native/                           # Rust FFI crate (notmux_mobile_native)
│   └── src/
│       ├── lib.rs
│       ├── api/
...
- **notmux-core** (workspace crate) — Shared types, RemoteClient, ThemeColors
