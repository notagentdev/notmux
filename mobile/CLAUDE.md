...
Remote terminal client for Android/iOS. Connects to the Vryn desktop server via REST + WebSocket. Uses `alacritty_terminal` in Rust for ANSI processing — identical terminal emulation as the desktop app.
...
└──────────┼──────────────────────────┼─────────────────────────┘
           ...
           ▼                          ▼
┌──────────────────────────────────────────────────────────────┐
│  Vryn Desktop (remote server on src/remote/)                │
...
├── native/                           # Rust FFI crate (vryn_mobile_native)
│   └── src/
│       ├── lib.rs
│       ├── api/
...
- **vryn-core** (workspace crate) — Shared types, RemoteClient, ThemeColors
