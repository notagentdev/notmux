...
impl Sidebar {
        ...
        cx.spawn(async move |sidebar_weak, cx| {
            ...
            let prep_result = smol::unblock(move || -> Result<(String, std::path::PathBuf, String, String, Option<String>), String> {
                ...
                let git_root = main_repo_path
                    ...
                    .or_else(|| vryn_git::get_repo_root(&project_path))
                    .ok_or_else(|| "Not a git repository".to_string())?;

                // Compute subdir (project path relative to git root)
                let normalized_project = vryn_git::repository::normalize_path(&project_path);
                let normalized_root = vryn_git::repository::normalize_path(&git_root);
                ...
                let branch = vryn_git::branch_names::generate_branch_name(&git_root);

                // Fast local lookup for default branch (no network)
                let default_branch = vryn_git::repository::get_default_branch(&git_root);

                // Compute target paths
                let (worktree_path, project_path) = vryn_git::repository::compute_target_paths(
            ...
            let create_result = smol::unblock(move || -> Result<(), String> {
                ...
                if let Some(ref db) = default_branch
                    && let Some(repo_str) = git_root_clone.to_str() {
                        let _ = vryn_core::process::safe_output(
                            vryn_core::process::command("git")
                                .args(["-C", repo_str, "fetch", "origin", db.as_str()]),
                        );
                    }

                vryn_git::repository::create_worktree_with_start_point(
                    &git_root_clone,
                    &branch_clone,
                    &target,
    ...
    fn push_group_cursor_items(
        &self,
        project_id: &str,
        layout: &Option<vryn_workspace::state::LayoutNode>,
        service_names: &HashMap<String, Vec<String>>,
        hook_terminal_ids: &HashMap<String, Vec<String>>,
        cursor_items: &mut Vec<SidebarCursorItem>,
    ) {
...
pub struct SidebarServiceInfo {
    pub name: String,
    pub status: vryn_services::manager::ServiceStatus,
    pub ports: Vec<u16>,
...
pub struct SidebarHookInfo {
    ...
    pub label: String,
    pub status: vryn_workspace::state::HookTerminalStatus,
    pub command: String,
    pub cwd: String,
}
...
pub struct SidebarProjectInfo {
    ...
    /// Services defined in vryn.yaml for this project
...
impl SidebarProjectInfo {
    pub(crate) fn from_project(project: &ProjectData) -> Self {
        ...
        let name = if project.worktree_info.is_some() {
            vryn_git::get_git_status(std::path::Path::new(&project.path))
                .and_then(|s| s.branch)
