...
impl SessionBackend {
    ...
    /// Load from environment variable NOTMUX_SESSION_BACKEND
    /// Defaults to Auto if not set
    #[allow(dead_code)]
    pub fn from_env() -> Self {
        std::env::var("NOTMUX_SESSION_BACKEND")
            .map(|s| Self::parse_backend(&s))
            .unwrap_or_default()
    }
...
#[cfg(windows)]
const WSL_DTACH_SOCKET_DIR: &str = "/tmp/notmux-dtach";
...
fn get_dtach_socket_dir() -> std::path::PathBuf {
    ...
    if let Ok(runtime_dir) = std::env::var("XDG_RUNTIME_DIR") {
        std::path::PathBuf::from(runtime_dir).join("notmux")
    } else {
        // Fallback: /tmp/notmux-<uid> for security
        #[cfg(unix)]
        {
            let uid = unsafe { libc::getuid() };
            std::path::PathBuf::from(format!("/tmp/notmux-{}", uid))
        }
        #[cfg(not(unix))]
        {
            std::env::temp_dir().join("notmux")
        }
    }
}
...
mod tests {
    ...
    fn test_build_wsl_session_command_dtach() {
        ...
        assert!(
            inner_cmd.contains("/tmp/notmux-dtach/"),
