...
//!
//! Path layout: `{project_dir}/.notmux/scrollback/{snapshot_key}.snap`.

use std::path::{Path, PathBuf};

const SNAPSHOT_EXT: &str = "snap";
const SNAPSHOT_DIR: &str = ".notmux/scrollback";

/// File-format magic for notmux scrollback snapshots. Header layout:
...
mod tests {
    ...
    fn load_rejects_legacy_grid_snapshots() {
        let dir = std::env::temp_dir().join(format!("notmux-snapshot-test-{}", uuid::Uuid::new_v4()));
    ...
    fn merge_existing_with_capture_keeps_last_n_lines_across_restarts() {
        let dir =
            std::env::temp_dir().join(format!("notmux-snapshot-merge-test-{}", uuid::Uuid::new_v4()));
        std::fs::create_dir_all(&dir).unwrap();
