...
use notmux_workspace::state::{SplitDirection, Workspace};
...
pub trait ActionDispatchClone: Send + Sync {
    fn dispatch_action(&self, action: notmux_core::api::ActionRequest, cx: &mut gpui::App);
        ...
        DragState::Split {
            ...
            if let Some(dispatcher) = action_dispatcher {
                dispatcher.dispatch_action(
                    notmux_core::api::ActionRequest::UpdateSplitSizes {
                        project_id,
                        path: layout_path,
                        sizes: new_sizes,
                    },
    ...
    ResizeHandle::new(
        ...
        move |mouse_pos, cx| {
            ...
            let (initial_sizes, visible_sizes_sum) = ws
                ...
                .and_then(|node| {
                    if let notmux_workspace::state::LayoutNode::Split {
                        sizes, children, ..
                    } = node
                    {
...
pub fn render_project_divider(
    ...
    ResizeHandle::new(false, t.border, t.border_active, move |mouse_pos, cx| {
        let bounds = *container_bounds.borrow();
        let num_projects = project_ids.len();
