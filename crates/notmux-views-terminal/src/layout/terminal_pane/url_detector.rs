...
impl UrlDetector {
    ...
    pub fn open_url(url: &str) {
        ...
        #[cfg(target_os = "linux")]
        {
            let _ = notmux_core::process::command("xdg-open").arg(url).spawn();
        }
        #[cfg(target_os = "macos")]
        {
            let _ = notmux_core::process::command("open").arg(url).spawn();
        }
        #[cfg(target_os = "windows")]
        {
            let _ = notmux_core::process::command("cmd")
                .args(["/C", "start", "", url])
                .spawn();
        }
    }
    ...
    pub fn open_file(path: &str, file_line: Option<u32>, file_col: Option<u32>, opener: &str) {
        ...
        if opener.is_empty() {
            ...
            #[cfg(target_os = "linux")]
            {
                let _ = notmux_core::process::command("xdg-open")
                    .arg(clean_path)
                    .spawn();
            }
            #[cfg(target_os = "macos")]
            {
                let _ = notmux_core::process::command("open").arg(clean_path).spawn();
            }
            #[cfg(target_os = "windows")]
            {
                let _ = notmux_core::process::command("cmd")
        ...
        match opener {
            "code" | "cursor" => {
                ...
                args.push(loc);
                let _ = notmux_core::process::command(opener).args(&args).spawn();
            }
            "zed" => {
                // Zed: file:line
                let mut loc = clean_path.to_string();
                ...
                let _ = notmux_core::process::command("zed").arg(&loc).spawn();
            }
            "subl" | "sublime" => {
                ...
                let _ = notmux_core::process::command("subl").arg(&loc).spawn();
            }
            "vim" | "nvim" => {
                // vim/nvim: +line file
            ...
            _ => {
                ...
                let _ = notmux_core::process::command(opener).arg(&loc).spawn();
            }
        }
    }
}
