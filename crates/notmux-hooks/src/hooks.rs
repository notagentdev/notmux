...
impl HookRunner {
    ...
    fn create_hook_terminal(
        ...
        let terminal_id = if keep_alive {
            ...
            // matching __notmux_hook_exit:<code> and updates hook status.
            script.push_str("; __notmux_rc=$?; printf '\\033]0;__notmux_hook_exit:%d\\007' \"$__notmux_rc\"; exec \"${SHELL:-sh}\"");
...
/// Includes base project vars and, for worktree projects, NOTMUX_BRANCH.
pub fn terminal_hook_env(
    ...
    if is_worktree {
        ...
        let branch = notmux_git::get_git_status(path)
            .and_then(|s| s.branch)
            .or_else(|| notmux_git::get_current_branch(path));
        if let Some(branch) = branch {
            env.insert("NOTMUX_BRANCH".into(), branch);
        }
    }
    env
}
...
fn build_headless_command(
    ...
    #[cfg(unix)]
    let mut cmd = notmux_core::process::command("sh");
    #[cfg(unix)]
    cmd.arg("-c").arg(command);

    #[cfg(windows)]
    let mut cmd = notmux_core::process::command("cmd");
    #[cfg(windows)]
    cmd.arg("/C").arg(command);
    ...
    if let Some(path) = env_vars.get("NOTMUX_PROJECT_PATH") {
...
fn project_env(
    ...
    env.insert("NOTMUX_PROJECT_ID".into(), project_id.into());
    env.insert("NOTMUX_PROJECT_NAME".into(), project_name.into());
    env.insert("NOTMUX_PROJECT_PATH".into(), project_path.into());
    if let Some(id) = folder_id {
        env.insert("NOTMUX_FOLDER_ID".into(), id.into());
    }
    if let Some(name) = folder_name {
        env.insert("NOTMUX_FOLDER_NAME".into(), name.into());
    }
    env
}
...
pub fn fire_on_worktree_create(
    ...
    if let Some(cmd) = resolve_hook(project_hooks, global_hooks, |h| &h.worktree.on_create) {
        ...
        env.insert("NOTMUX_BRANCH".into(), branch.into());
...
fn merge_env(
    ...
    env.insert("NOTMUX_BRANCH".into(), branch.into());
    env.insert("NOTMUX_TARGET_BRANCH".into(), target_branch.into());
    env.insert("NOTMUX_MAIN_REPO_PATH".into(), main_repo_path.into());
    env
}
...
pub fn fire_on_rebase_conflict(
    ...
    if let Some(cmd) = resolve_hook(project_hooks, global_hooks, |h| {
        ...
        env.insert("NOTMUX_REBASE_ERROR".into(), rebase_error.into());
        log::info!(
...
pub fn fire_terminal_on_close(
    ...
    if let Some(cmd) = resolve_hook_with_parent(project_hooks, parent_hooks, global_hooks, |h| {
        ...
        env.insert("NOTMUX_TERMINAL_ID".into(), terminal_id.into());
        if let Some(name) = terminal_name {
            env.insert("NOTMUX_TERMINAL_NAME".into(), name.into());
        }
        if let Some(code) = exit_code {
            env.insert("NOTMUX_EXIT_CODE".into(), code.to_string());
        }
        if is_worktree {
            let path = std::path::Path::new(project_path);
            let branch = notmux_git::get_git_status(path)
...
mod tests {
    use super::*;
    use notmux_state::WorktreeHooks;
    ...
    fn build_hook_label_uses_branch() {
        let mut env = HashMap::new();
        env.insert("NOTMUX_BRANCH".into(), "feature/foo".into());
        assert_eq!(
    ...
    fn resolve_hook_with_parent_three_tier() {
        use notmux_state::TerminalHooks;
    ...
    #[test]
    fn valid_env_keys() {
        assert!(is_valid_env_key("NOTMUX_PROJECT_PATH"));
    ...
    fn apply_on_create_with_env_vars() {
        ...
        let mut env = HashMap::new();
        env.insert("NOTMUX_PROJECT_ID".into(), "proj-123".into());
        let result = apply_on_create(&shell, "echo hello", &env);
        match &result {
            ShellType::Custom { path: _, args } => {
                let cmd = &args[1];
                assert!(cmd.contains("export NOTMUX_PROJECT_ID="), "got: {}", cmd);
    ...
    fn apply_shell_wrapper_with_env_vars() {
        ...
        let mut env = HashMap::new();
        env.insert("NOTMUX_PROJECT_NAME".into(), "my-project".into());
        let result = apply_shell_wrapper(&shell, "wrapper {shell}", &env);
        match &result {
            ShellType::Custom { path: _, args } => {
                let cmd = &args[1];
                assert!(cmd.contains("export NOTMUX_PROJECT_NAME="), "got: {}", cmd);
