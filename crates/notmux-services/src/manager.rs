use crate::config::{ServiceDefinition, load_project_config};
...
pub struct ServiceInstance {
    ...
    /// For NotMux services: the process PTY. For Docker services: the log viewer PTY (ephemeral).
    pub terminal_id: Option<String>,
    pub restart_count: u32,
    pub detected_ports: Vec<u16>,
    /// Docker service not listed in notmux.yaml filter — shown in "Other" section.
    pub is_extra: bool,
}
...
impl ServiceManager {
    ...
    /// Parse `notmux.yaml` for a project, create `ServiceInstance` entries,
    ...
    pub fn load_project_services(
        ...
        let config = match load_project_config(project_path) {
            Ok(Some(config)) => {
                log::info!(
                    "[services] Found notmux.yaml with {} services",
                    config.services.len()
                );
                config
            }
            Ok(None) => {
                log::info!("[services] No notmux.yaml found at {}", project_path);
                // No notmux.yaml — still try Docker Compose auto-detection
                self.project_paths
            ...
            Err(e) => {
                log::error!("Failed to load notmux.yaml for project {}: {}", project_id, e);
                return;
            }
        };
            ...
            .reconnect_terminal(saved_terminal_id, &cwd, Some(&shell))
        {
            Ok(terminal_id) => {
                ...
                let instance = self
                    .instances
                    .get_mut(&key)
                    .expect("key verified present at function entry");
    ...
    pub fn service_terminal_ids(&self, project_id: &str) -> HashMap<String, String> {
        self.instances
            .iter()
            .filter(|((pid, _), inst)| pid == project_id && inst.kind == ServiceKind::NotMux)
    ...
    /// Re-read `notmux.yaml`. Stop removed services, add new ones,
    ...
    pub fn reload_project_services(
        ...
        let new_config = match load_project_config(project_path) {
            ...
            Err(e) => {
                log::error!(
                    "Failed to reload notmux.yaml for project {}: {}",
                    project_id,
                    e
                );
                return;
        ...
        // Stop and remove NotMux services that no longer exist in config
            ...
            .filter(|(pid, name)| {
                pid == project_id
                    ...
                    && self
                        ...
                        .is_some_and(|i| i.kind == ServiceKind::NotMux)
            })
            .cloned()
            .collect();
    ...
    /// Start a service by spawning a PTY (NotMux) or running `docker compose start` (Docker).
    pub fn start_service(
        ...
        match &instance.kind {
            ServiceKind::DockerCompose { compose_file } => {
                ...
                cx.spawn(async move |this: WeakEntity<ServiceManager>, cx| {
                    let result = cx
                        .background_executor()
                        .spawn(async move {
                            let mut cmd = notmux_core::process::command("docker");
                            cmd.args(["compose", "-f", &compose_file, "start", &name])
                                .current_dir(&path);
                            notmux_core::process::safe_output(&mut cmd)
                        })
                        .await;
            ...
            ServiceKind::NotMux => {
                self.start_notmux_service(project_id, service_name, project_path, cx);
            }
        }
    }

    /// Start an NotMux service by spawning a PTY with the service command.
    fn start_notmux_service(
        &mut self,
        project_id: &str,
        service_name: &str,
        project_path: &str,
    ...
    pub fn stop_service(&mut self, project_id: &str, service_name: &str, cx: &mut Context<Self>) {
        let key = (project_id.to_string(), service_name.to_string());
        let instance = match self.instances.get_mut(&key) {
            Some(i) => i,
            None => return,
                    ...
                    cx.background_executor()
                        .spawn(async move {
                            ...
                            cmd.args(["compose", "-f", &compose_file, "stop", &name])
                                .current_dir(&path);
                            let _ = notmux_core::process::safe_output(&mut cmd);
                        })
                        .await;
                })
                .detach();
    ...
    pub fn start_all(&mut self, project_id: &str, project_path: &str, cx: &mut Context<Self>) {
    ...
    /// Whether the project has any service definitions loaded (NotMux or Docker).
    ...
    fn start_docker_status_poller(
        ...
        cx.spawn(async move |this: WeakEntity<ServiceManager>, cx| {
            ...
            loop {
                ...
                let result =
                    smol::unblock(move || docker_compose::poll_status(&path_clone, &file_clone))
                        .await;
...
mod tests {
    ...
    fn docker_service_terminal_ids_excluded() {
        ...
        let ids: HashMap<String, String> = instances
            .iter()
            .filter(|((pid, _), inst)| pid == "proj1" && inst.kind == ServiceKind::NotMux)
            .filter_map(|((_, name), instance)| {
                instance
                    .terminal_id
                    .as_ref()
