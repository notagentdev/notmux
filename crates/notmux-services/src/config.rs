...
pub struct NotMuxProjectConfig {
    #[serde(default)]
...
/// Load project config from `{project_path}/notmux.yaml`.
///
/// Returns `Ok(None)` if the file doesn't exist, `Err` on parse failure.
pub fn load_project_config(project_path: &str) -> Result<Option<NotMuxProjectConfig>, String> {
    let path = Path::new(project_path).join("notmux.yaml");
    if !path.exists() {
        return Ok(None);
    }
    let content = std::fs::read_to_string(&path)
        .map_err(|e| format!("Failed to read {}: {}", path.display(), e))?;
    let config: NotMuxProjectConfig = serde_yaml::from_str(&content)
