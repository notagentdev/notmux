...
use vryn_ui::tokens::{ui_text_sm, ui_text_md};
...
use vryn_workspace::state::{LayoutNode, SplitDirection};
use gpui::*;
...
pub(super) struct TabActionContext<D: ActionDispatch> {
    pub workspace: Entity<vryn_workspace::state::Workspace>,
                ...
                header_button_base(HeaderAction::SplitVertical, &id_suffix, ButtonSize::COMPACT, &t, None, None)
                    .on_click(move |_, _window, cx| {
                        if let Some(ref dispatcher) = ctx_split_v.action_dispatcher {
                            dispatcher.dispatch(vryn_core::api::ActionRequest::SplitTerminal {
                        ...
                        move |_, _window, cx| {
                            if let Some(ref tid) = terminal_id_for_minimize
                                && let Some(ref dispatcher) = ctx_minimize.action_dispatcher {
                                    dispatcher.dispatch(vryn_core::api::ActionRequest::ToggleMinimized {
                    ...
                    .on_click(move |_, _window, cx| {
                        if let Some(ref tid) = terminal_id_for_fullscreen
                            && let Some(ref dispatcher) = ctx_fullscreen.action_dispatcher {
                                dispatcher.dispatch(vryn_core::api::ActionRequest::SetFullscreen {
                                    ...
                                    h_flex()
                                        ...
                                        .on_click(move |_, _window, cx| {
                                            ...
                                            if let Some(ref dispatcher) = close_dispatcher {
                                                dispatcher.dispatch(
                                                    vryn_core::api::ActionRequest::CloseTerminal {
                    ...
                    cx.listener(move |this, event: &MouseDownEvent, _window, cx| {
                        this.request_broker.update(cx, |broker, cx| {
                            broker.push_overlay_request(
                                vryn_workspace::requests::OverlayRequest::TabContextMenu {
                                    tab_index: i,
                    ...
                    cx.listener(move |_this, _event: &MouseDownEvent, _window, cx| {
                        if let Some(ref tid) = terminal_id
                            && let Some(ref dispatcher) = action_dispatcher {
                                dispatcher.dispatch(vryn_core::api::ActionRequest::CloseTerminal {
                                    project_id: project_id.clone(),
                                    terminal_id: tid.clone(),
                                }, cx);
                            }
                    ...
                    .on_drop(cx.listener({
                        ...
                        move |this, drag: &PaneDrag, _window, cx| {
                                ...
                                && drag_parent == layout_path_for_drop.as_slice()
                            {
                                if let Some(from_index) = drag_tab_index
                                    && from_index != i {
                                        ...
                                        if let Some(ref dispatcher) = dispatcher_for_drop {
                                            dispatcher.dispatch(vryn_core::api::ActionRequest::MoveTab {
                                ...
                                dispatcher.dispatch(vryn_core::api::ActionRequest::MoveTerminalToTabGroup {
                    ...
                    cx.listener(move |this, _, window, cx| {
                        ...
                        if !standalone
                            && let Some(ref dispatcher) = dispatcher_for_click {
                                dispatcher.dispatch(vryn_core::api::ActionRequest::SetActiveTab {
                                    project_id: project_id.clone(),
