...
/// Load project config from `{project_path}/vryn.yaml`.
///
/// Returns `Ok(None)` if the file doesn't exist, `Err` on parse failure.
pub fn load_project_config(project_path: &str) -> Result<Option<VrynProjectConfig>, String> {
    let path = Path::new(project_path).join("vryn.yaml");
    if !path.exists() {
        return Ok(None);
    }
...
services:
  - name: "Vite Dev"
    command: "npm run dev"
"#;
        let config: VrynProjectConfig = serde_yaml::from_str(yaml).unwrap();
