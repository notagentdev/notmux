...
pub struct ServiceInstance {
    ...
    /// For Vryn services: the process PTY. For Docker services: the log viewer PTY (ephemeral).
    pub terminal_id: Option<String>,
    pub restart_count: u32,
    pub detected_ports: Vec<u16>,
    /// Docker service not listed in vryn.yaml filter — shown in "Other" section.
    pub is_extra: bool,
}
...
impl ServiceManager {
    ...
    /// Parse `vryn.yaml` for a project, create `ServiceInstance` entries,
    ...
    pub fn load_project_services(
        ...
        let config = match load_project_config(project_path) {
            Ok(Some(config)) => {
                log::info!(
                    "[services] Found vryn.yaml with {} services",
                    config.services.len()
                );
                config
            }
            Ok(None) => {
                log::info!("[services] No vryn.yaml found at {}", project_path);
                // No vryn.yaml — still try Docker Compose auto-detection
                self.project_paths
            ...
            Err(e) => {
                log::error!("Failed to load vryn.yaml for project {}: {}", project_id, e);
                return;
            }
        };
    ...
    pub fn service_terminal_ids(&self, project_id: &str) -> HashMap<String, String> {
        self.instances
            .iter()
            .filter(|((pid, _), inst)| pid == project_id && inst.kind == ServiceKind::Vryn)
    ...
    /// Re-read `vryn.yaml`. Stop removed services, add new ones,
    ...
    pub fn reload_project_services(
        ...
        let new_config = match load_project_config(project_path) {
            ...
            Err(e) => {
                log::error!(
                    "Failed to reload vryn.yaml for project {}: {}",
                    project_id,
                    e
                );
                return;
        ...
        // Stop and remove Vryn services that no longer exist in config
            ...
            .filter(|(pid, name)| {
                pid == project_id
                    ...
                    && self
                        ...
                        .is_some_and(|i| i.kind == ServiceKind::Vryn)
            })
            .cloned()
            .collect();
    ...
    /// Start a service by spawning a PTY (Vryn) or running `docker compose start` (Docker).
    pub fn start_service(
        ...
        match &instance.kind {
            ServiceKind::DockerCompose { compose_file } => {
                ...
                cx.spawn(async move |this: WeakEntity<ServiceManager>, cx| {
                    let result = cx
                        .background_executor()
                        .spawn(async move {
                            let mut cmd = vryn_core::process::command("docker");
                            cmd.args(["compose", "-f", &compose_file, "start", &name])
                                .current_dir(&path);
                            vryn_core::process::safe_output(&mut cmd)
                        })
                        .await;
            ...
            ServiceKind::Vryn => {
                self.start_vryn_service(project_id, service_name, project_path, cx);
            }
        }
    }

    /// Start an Vryn service by spawning a PTY with the service command.
    fn start_vryn_service(
        &mut self,
        project_id: &str,
        service_name: &str,
        project_path: &str,
                    ...
                    cx.background_executor()
                        .spawn(async move {
                            ...
                            cmd.args(["compose", "-f", &compose_file, "stop", &name])
                                .current_dir(&path);
                            let _ = vryn_core::process::safe_output(&mut cmd);
                        })
                        .await;
                })
                .detach();
    ...
    /// Whether the project has any service definitions loaded (Vryn or Docker).
...
mod tests {
    ...
    fn docker_service_terminal_ids_excluded() {
        ...
        let ids: HashMap<String, String> = instances
            .iter()
            .filter(|((pid, _), inst)| pid == "proj1" && inst.kind == ServiceKind::Vryn)
            .filter_map(|((_, name), instance)| {
                instance
                    .terminal_id
                    .as_ref()
