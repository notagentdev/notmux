...
//! them to Okena's `ThemeColors`. Users can download a theme from the
//! VS Code extension source, drop the JSON into `~/.config/okena/themes/`,
...
//!
//! Ignored (not in scope): `tokenColors`, `semanticTokenColors` (Okena has
...
/// Okena native themes use flat keys like `bg_primary`.
...
/// Map a VS Code theme to Okena's flat `ThemeColors`, falling back to `fallback`
/// for any Okena field that has no matching VS Code key.
///
/// VS Code themes heavily rely on alpha-blended colors for subtle UI tints
/// (selections, hovers, borders). Because Okena's `ThemeColors` stores
...
mod tests {
    ...
    #[test]
    fn rejects_okena_native_schema() {
        let v: serde_json::Value =
