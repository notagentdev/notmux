...
//! them to NotMux's `ThemeColors`. Users can download a theme from the
//! VS Code extension source, drop the JSON into `~/.config/notmux/themes/`,
...
//!
//! Token colors are mapped into NotMux's syntax highlighting slots so the
...
/// NotMux native themes use flat keys like `bg_primary`.
...
/// Map a VS Code theme to NotMux's flat `ThemeColors`, falling back to `fallback`
/// for any NotMux field that has no matching VS Code key.
///
/// VS Code themes heavily rely on alpha-blended colors for subtle UI tints
/// (selections, hovers, borders). Because NotMux's `ThemeColors` stores
...
mod tests {
    ...
    #[test]
    fn rejects_notmux_native_schema() {
        let v: serde_json::Value =
    ...
    fn include_chain_merges_with_child_precedence() {
        let tmp = std::env::temp_dir().join(format!("notmux-vscode-test-{}", std::process::id()));
    ...
    fn include_loop_guard_triggers() {
        let tmp = std::env::temp_dir().join(format!("notmux-vscode-loop-{}", std::process::id()));
        std::fs::create_dir_all(&tmp).unwrap();
