...
use notmux_extensions::ThemeColors;
        ...
        match info.status() {
            UpdateStatus::Ready { version, .. } => {
                let release_url = format!(
                    "https://github.com/notagentdev/notmux/releases/tag/v{}",
                    version
                );
                h_flex()
                    .id("update-ready")
                ...
                div()
                    ...
                    .child(
                        div()
                            .text_color(rgb(t.text_muted))
                            .text_size(ui_text_sm(cx))
                            .child(format!("v{} — brew upgrade notmux", version)),
                    )
                    .child(
                        div()
                            .id("update-dismiss")
