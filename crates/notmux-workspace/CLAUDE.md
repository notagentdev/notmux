# notmux-workspace — Workspace coordinator (GPUI entity)

GPUI entity that wires together persistent data (`notmux-state`), layout
algorithms (`notmux-layout`), hook execution (`notmux-hooks`), and persistence.

## Layered crates

| Crate | Role |
|-------|------|
| `notmux-state` | Pure data: `WorkspaceData`, `ProjectData`, `FolderData`, `WorktreeMetadata`, `HookTerminalEntry`, `HooksConfig`, `Toast`. No GPUI. |
| `notmux-layout` | `LayoutNode` recursive tree + tree algorithms (split/normalize/merge_visual_state). |
| `notmux-hooks` | `HookRunner` (PTY) + `HookMonitor`. Decoupled from `notmux-workspace` — receives metadata in, returns `HookTerminalResult`. |
| `notmux-workspace` | This crate — `Workspace` entity, `actions/`, persistence, settings, sessions. |
...
- `Workspace` (GPUI entity in `state.rs`) — coordinator over `WorkspaceData` from `notmux-state`. Holds focus, lifecycle, remote-sync, access-history.
...
| File | Purpose |
|------|---------|
| `state.rs` | `Workspace` GPUI entity + tests (data types live in `notmux-state`) |
| `persistence.rs` | Load/save `workspace.json`. Validation, migration, layout normalization on load. |
| `settings.rs` | `AppSettings` schema, debounced auto-save. Re-exports `HooksConfig` from `notmux-state`. |
| `hooks.rs` / `hook_monitor.rs` | Re-exports the hook execution surface from `notmux-hooks`. |
