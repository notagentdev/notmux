...
impl Workspace {
    ...
    pub fn add_project(
        ...
        #[cfg(windows)]
        let default_shell =
            notmux_terminal::shell_config::parse_wsl_unc_path(&path).map(|(distro, _)| {
                notmux_terminal::shell_config::ShellType::Wsl {
                    distro: Some(distro),
                }
            });
        #[cfg(not(windows))]
        let default_shell: Option<notmux_terminal::shell_config::ShellType> = None;
    ...
    pub fn create_worktree_project(
        ...
        notmux_git::create_worktree(repo_path, branch, &target, create_branch)?;
    ...
    pub fn fire_worktree_hooks(
        ...
        let branch = notmux_git::repository::get_current_branch(std::path::Path::new(&path))
    ...
    /// Does NOT fire hooks (the worktree was created outside NotMux).
    ...
    pub fn add_discovered_worktree(
        ...
        let (_git_root, subdir) =
            notmux_git::resolve_git_root_and_subdir(std::path::Path::new(&parent_path));
        let project_path = notmux_git::repository::project_path_in_worktree(wt_path, &subdir);
    ...
    pub fn remove_worktree_project(
        ...
        let worktree_path = notmux_git::get_repo_root(&project_pathbuf).unwrap_or(project_pathbuf);

        // Resolve branch BEFORE removal (git worktree remove deletes the checkout)
        let branch = notmux_git::get_current_branch(&worktree_path).unwrap_or_default();
        ...
        notmux_git::remove_worktree(&worktree_path, force)?;
