...
///
/// - Release builds: `~/.notmux`
/// - Debug builds (`cargo run`, `cargo test`): `~/.notmux-dev`
///
/// One central, hidden, well-known directory on every OS — Unix-like systems
/// use `~/.<name>` (`~/.notmux`), Windows resolves `$HOME` via
/// `dirs::home_dir()` to `%USERPROFILE%`. The user can find and reset
/// the entire app state with a single `rm -rf ~/.notmux(-dev)`; this is
/// the anti-Registry design choice (no XDG, no Apple Application Support,
/// no scattered OS-specific paths).
///
...
///
/// `NOTMUX_CONFIG_DIR` overrides both defaults for ad-hoc testing
/// (e.g. spinning up a third isolated instance). Unset for normal users.
pub fn get_config_dir() -> PathBuf {
    if let Some(dir) = std::env::var_os("NOTMUX_CONFIG_DIR")
        && !dir.is_empty()
    {
        return PathBuf::from(dir);
    }
    #[cfg(debug_assertions)]
    let dir_name = ".notmux-dev";
    #[cfg(not(debug_assertions))]
    let dir_name = ".notmux";
    dirs::home_dir()
        .unwrap_or_else(|| {
            // Don't fall back to "." — that's a relative path and would silently
            // scatter config across whatever CWD is when no $HOME is set.
            log::error!("notmux: $HOME/USERPROFILE not set; config inaccessible");
            PathBuf::from("/")
        })
        .join(dir_name)
}
...
pub fn acquire_instance_lock() -> Result<LockGuard> {
    let lock_path = get_config_dir().join("notmux.lock");
        ...
        && let Ok(pid) = content.trim().parse::<u32>()
    {
        if is_process_alive(pid) {
            anyhow::bail!(
                "Another NotMux instance is already running (PID {pid}). \
    ...
    for project in &mut data.projects {
        if project.default_shell.is_none() {
            if let Some((distro, _)) =
                notmux_terminal::shell_config::parse_wsl_unc_path(&project.path)
            {
                project.default_shell = Some(notmux_terminal::shell_config::ShellType::Wsl {
                    distro: Some(distro),
                });
            }
        }
        ...
        for project in &mut data.projects {
            #[cfg(windows)]
            {
                ...
                if let Some(ShellType::Wsl { distro }) = &project.default_shell {
                    let wsl_backend = notmux_terminal::session_backend::resolve_for_wsl(
                        distro.as_deref(),
                        backend_preference,
                    );
...
mod tests {
    ...
    fn validate_clear_terminal_ids() {
        ...
        project.layout = Some(LayoutNode::Terminal {
            ...
            minimized: true,
            detached: true,
            shell_type: notmux_terminal::shell_config::ShellType::Default,
            zoom_level: 1.0,
        });
        project
            .service_terminals
