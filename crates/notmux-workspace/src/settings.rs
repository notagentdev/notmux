...
// Hook configuration types live in `notmux-state` to keep them GPUI-free.
pub use notmux_state::{HooksConfig, ProjectHooks, TerminalHooks, WorktreeHooks};
...
fn default_file_explorer_show_hidden() -> bool {
    true
}
...
pub struct FileExplorerSettings {
    ...
    pub width: f32,
    /// Whether to show hidden (dotfile) files and directories in the tree.
    /// `.git` is always hidden regardless of this flag. Defaults to `true`
    /// so dotfiles like `.env`, `.gitignore`, `.vscode/` are visible out
    /// of the box.
    #[serde(default = "default_file_explorer_show_hidden")]
...
pub struct AppSettings {
    ...
    /// Diff viewer font size (default: 12.0)
...
fn recover_settings_from_json(content: &str) -> Result<AppSettings> {
    ...
    if let Some(v) = obj.get("diff_font_size").and_then(|v| v.as_f64()) {
        settings.diff_font_size = (v as f32).clamp(8.0, 24.0);
    }
...
mod tests {
    ...
    #[test]
    fn file_explorer_show_hidden_defaults_on() {
        // Empty JSON: `file_explorer` is missing entirely, so the whole
        // sub-struct falls back to its `Default` impl.
        let settings: AppSettings = serde_json::from_str("{}").unwrap();
        assert!(settings.file_explorer.show_hidden);
        // Explicit empty `file_explorer` object: the `show_hidden` field is
        // missing, so the per-field serde default fn must kick in and yield
        // `true` (not `<bool as Default>` which would be `false`).
        let settings: AppSettings =
            serde_json::from_str(r#"{"file_explorer": {}}"#).unwrap();
        assert!(settings.file_explorer.show_hidden);
    }
    ...
    fn terminal_working_directory_uses_first_project_when_configured() {
        ...
        let project = first.join("missing-notmux-project-dir");
    ...
    fn terminal_working_directory_invalid_fixed_path_falls_back_to_project() {
        ...
        let resolved = TerminalWorkingDirectory::Always {
            directory: "/definitely/not/a/real/notmux/path".to_string(),
        }
