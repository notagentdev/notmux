...
/// that flavor wires syntax colors to the current NotMux theme instead of a
...
/// Default text color derived from the current NotMux theme.
pub fn default_text_color_for(colors: &ThemeColors) -> Rgba {
    rgb_to_rgba(colors.text_primary)
}

/// Build a syntect `Theme` whose syntax colors are sourced from NotMux's
/// active `ThemeColors`.
...
/// * `colors` - Active NotMux theme colors, including syntax colors
