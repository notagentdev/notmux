...
impl GitProvider for LocalGitProvider {
    fn is_git_repo(&self) -> bool {
        notmux_git::is_git_repo(std::path::Path::new(&self.path))
    }

    fn get_diff(&self, mode: DiffMode, ignore_whitespace: bool) -> Result<DiffResult, String> {
        notmux_git::get_diff_with_options(std::path::Path::new(&self.path), mode, ignore_whitespace)
    }
    ...
    fn get_diff_for_file(
        ...
        notmux_git::get_diff_for_file_with_options(
    ...
    fn get_file_contents(
        ...
        notmux_git::get_file_contents_for_diff(std::path::Path::new(&self.path), file_path, mode)
    }
    ...
    fn list_branches(&self) -> Vec<String> {
        notmux_git::list_branches(std::path::Path::new(&self.path))
    }

    fn get_working_tree_status(&self) -> WorkingTreeStatus {
        notmux_git::get_working_tree_status(std::path::Path::new(&self.path))
    }
    ...
    fn get_file_statuses(&self, rel_paths: &[String]) -> Vec<FileStatusRefresh> {
        notmux_git::get_file_statuses(std::path::Path::new(&self.path), rel_paths)
    }

    fn stage_file(&self, file_path: &str) -> Result<(), String> {
        notmux_git::stage_file(std::path::Path::new(&self.path), file_path)
    }
    ...
    fn stage_all(&self) -> Result<(), String> {
        notmux_git::stage_all(std::path::Path::new(&self.path))
    }

    fn unstage_all(&self) -> Result<(), String> {
        notmux_git::unstage_all(std::path::Path::new(&self.path))
    }

    fn discard_file(&self, file_path: &str, is_untracked: bool) -> Result<(), String> {
        notmux_git::discard_file(std::path::Path::new(&self.path), file_path, is_untracked)
    }

    fn commit(&self, message: &str, amend: bool, signoff: bool) -> Result<(), String> {
        notmux_git::commit(std::path::Path::new(&self.path), message, amend, signoff)
    }

    fn uncommit(&self) -> Result<(), String> {
        notmux_git::uncommit(std::path::Path::new(&self.path))
    }

    fn fetch(&self) -> Result<(), String> {
        notmux_git::fetch_all(std::path::Path::new(&self.path))
    }

    fn pull(&self) -> Result<(), String> {
        notmux_git::pull(std::path::Path::new(&self.path))
    }

    fn push(&self) -> Result<(), String> {
        let path = std::path::Path::new(&self.path);
        let branch = notmux_git::get_current_branch(path)
            .ok_or_else(|| "No current branch (detached HEAD?)".to_string())?;
        notmux_git::push_branch(path, &branch)
    }

    fn stash_all(&self) -> Result<(), String> {
        notmux_git::stash_all_including_untracked(std::path::Path::new(&self.path))
    }

    fn stash_pop(&self) -> Result<(), String> {
        notmux_git::stash_pop(std::path::Path::new(&self.path))
    }

    fn stash_list(&self) -> Result<Vec<StashEntry>, String> {
        notmux_git::stash_list(std::path::Path::new(&self.path))
    }

    fn stash_apply(&self, index: usize) -> Result<(), String> {
        notmux_git::stash_apply(std::path::Path::new(&self.path), index)
    }

    fn stash_drop(&self, index: usize) -> Result<(), String> {
        notmux_git::stash_drop(std::path::Path::new(&self.path), index)
    }

    fn stash_show_patch(&self, index: usize) -> Result<String, String> {
        notmux_git::stash_show_patch(std::path::Path::new(&self.path), index)
    }

    fn discard_all_tracked(&self) -> Result<(), String> {
        notmux_git::discard_all_tracked(std::path::Path::new(&self.path))
    }
}
...
impl GitProvider for RemoteGitProvider {
    ...
    fn get_diff(&self, mode: DiffMode, ignore_whitespace: bool) -> Result<DiffResult, String> {
        let action = notmux_core::api::ActionRequest::GitDiff {
    ...
    fn get_file_contents(
        ...
        let action = notmux_core::api::ActionRequest::GitFileContents {
            project_id: self.project_id.clone(),
    ...
    fn get_diff_file_summary(&self) -> Vec<FileDiffSummary> {
        let action = notmux_core::api::ActionRequest::GitDiffSummary {
            project_id: self.project_id.clone(),
        };
        match self.post_action(action) {
    ...
    fn list_branches(&self) -> Vec<String> {
        let action = notmux_core::api::ActionRequest::GitListBranches {
            project_id: self.project_id.clone(),
        };
        match self.post_action(action) {
            Ok(Some(value)) => serde_json::from_value(value).unwrap_or_else(|e| {
    ...
    fn get_working_tree_status(&self) -> WorkingTreeStatus {
        let action = notmux_core::api::ActionRequest::GitWorkingTreeStatus {
            project_id: self.project_id.clone(),
        };
        match self.post_action(action) {
            Ok(Some(value)) => serde_json::from_value(value).unwrap_or_else(|e| {
    ...
    fn stage_file(&self, file_path: &str) -> Result<(), String> {
        let action = notmux_core::api::ActionRequest::GitStageFile {
            project_id: self.project_id.clone(),
            file_path: file_path.to_string(),
        };
    ...
    fn unstage_file(&self, file_path: &str) -> Result<(), String> {
        let action = notmux_core::api::ActionRequest::GitUnstageFile {
            project_id: self.project_id.clone(),
            file_path: file_path.to_string(),
        };
        self.post_action(action).map(|_| ())
    ...
    fn stage_all(&self) -> Result<(), String> {
        let action = notmux_core::api::ActionRequest::GitStageAll {
            project_id: self.project_id.clone(),
        };
        self.post_action(action).map(|_| ())
    }
    ...
    fn unstage_all(&self) -> Result<(), String> {
        let action = notmux_core::api::ActionRequest::GitUnstageAll {
            project_id: self.project_id.clone(),
        };
        self.post_action(action).map(|_| ())
    }
    ...
    fn discard_file(&self, file_path: &str, is_untracked: bool) -> Result<(), String> {
        let action = notmux_core::api::ActionRequest::GitDiscardFile {
            project_id: self.project_id.clone(),
            file_path: file_path.to_string(),
            is_untracked,
        };
    ...
    fn commit(&self, message: &str, amend: bool, signoff: bool) -> Result<(), String> {
        let action = notmux_core::api::ActionRequest::GitCommit {
            project_id: self.project_id.clone(),
    ...
    fn uncommit(&self) -> Result<(), String> {
        let action = notmux_core::api::ActionRequest::GitUncommit {
            project_id: self.project_id.clone(),
        };
        self.post_action(action).map(|_| ())
    }
    ...
    fn fetch(&self) -> Result<(), String> {
        let action = notmux_core::api::ActionRequest::GitFetch {
            project_id: self.project_id.clone(),
        };
        self.post_action(action).map(|_| ())
    }
    ...
    fn pull(&self) -> Result<(), String> {
        let action = notmux_core::api::ActionRequest::GitPull {
            project_id: self.project_id.clone(),
        };
        self.post_action(action).map(|_| ())
    }
    ...
    fn push(&self) -> Result<(), String> {
        let action = notmux_core::api::ActionRequest::GitPush {
            project_id: self.project_id.clone(),
        };
        self.post_action(action).map(|_| ())
    }
