...
impl WorktreeListPopover {
    pub fn new(
        ...
        let project_path = workspace
            ...
            .unwrap_or_default();
        let (git_root, subdir) =
            notmux_git::resolve_git_root_and_subdir(std::path::Path::new(&project_path));
        let norm_git_root = notmux_git::repository::normalize_path(&git_root);
        let entries = notmux_git::repository::list_git_worktrees(&git_root);
    ...
    fn find_tracked_project_id(&self, wt_path: &str, cx: &App) -> Option<String> {
        let expected_path = notmux_git::repository::project_path_in_worktree(wt_path, &self.subdir);
...
impl Render for WorktreeListPopover {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let worktrees: Vec<(String, String, bool)> = self
            ...
            .iter()
            .filter(|(wt_path, _)| {
                let norm_wt = notmux_git::repository::normalize_path(std::path::Path::new(wt_path));
                norm_wt != self.norm_git_root
            })
            .map(|(wt_path, branch)| {
                let expected_path = notmux_git::repository::project_path_in_worktree(wt_path, subdir);
        ...
        let panel = notmux_ui::popover::popover_panel("worktree-list-panel", &t)
            .w(px(280.0))
            .max_h(px(400.0))
            .child(
                div()
