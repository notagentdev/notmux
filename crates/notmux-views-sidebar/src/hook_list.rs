...
impl Sidebar {
                ...
                move |this, _, _window, cx| {
                    ...
                    this.request_broker.update(cx, |broker, cx| {
                        broker.push_overlay_request(
                            notmux_workspace::requests::OverlayRequest::ShowHookTerminal {
                    ...
                    .child(
                        ...
                        .on_click(cx.listener({
                            ...
                            move |this, _, _window, cx| {
                                cx.stop_propagation();
                                if let Some(monitor) =
                                    cx.try_global::<notmux_workspace::hook_monitor::HookMonitor>()
                                {
    ...
    pub fn rerun_hook_terminal(
        ...
        let Some(runner) = cx
            .try_global::<notmux_workspace::hooks::HookRunner>()
            .cloned()
        else {
        ...
        match runner.backend.create_terminal(cwd, None) {
            Ok(new_terminal_id) => {
                ...
                let terminal = std::sync::Arc::new(notmux_terminal::terminal::Terminal::new(
