...
use notmux_services::manager::ServiceManager;
...
use notmux_workspace::state::{FolderData, ProjectData, Workspace};
...
impl Sidebar {
    pub fn new(
        ...
        sidebar.start_waiting_terminal_tick(cx);
        sidebar
    }

    fn start_waiting_terminal_tick(&self, cx: &mut Context<Self>) {
        cx.spawn(async move |this: WeakEntity<Sidebar>, cx| {
            loop {
                cx.background_executor().timer(Duration::from_secs(1)).await;

                let result = this.update(cx, |this, cx| {
                    if this.view == SidebarView::Projects && this.any_waiting_terminal() {
                        cx.notify();
                    }
                });
    ...
    fn any_waiting_terminal(&self) -> bool {
        let terminals = self.terminals.lock();
        terminals.values().any(|t| t.is_waiting_for_input())
    }
        ...
        cx.spawn(async move |sidebar_weak, cx| {
            ...
            let prep_result = smol::unblock(move || -> Result<(String, std::path::PathBuf, String, String, Option<String>), String> {
                ...
                let git_root = main_repo_path
                    ...
                    .or_else(|| notmux_git::get_repo_root(&project_path))
                    .ok_or_else(|| "Not a git repository".to_string())?;

                // Compute subdir (project path relative to git root)
                let normalized_project = notmux_git::repository::normalize_path(&project_path);
                let normalized_root = notmux_git::repository::normalize_path(&git_root);
                ...
                let branch = notmux_git::branch_names::generate_branch_name(&git_root);

                // Fast local lookup for default branch (no network)
                let default_branch = notmux_git::repository::get_default_branch(&git_root);

                // Compute target paths
                let (worktree_path, project_path) = notmux_git::repository::compute_target_paths(
            ...
            let create_result = smol::unblock(move || -> Result<(), String> {
                ...
                if let Some(ref db) = default_branch
                    && let Some(repo_str) = git_root_clone.to_str() {
                        let _ = notmux_core::process::safe_output(
                            notmux_core::process::command("git")
                                .args(["-C", repo_str, "fetch", "origin", db.as_str()]),
                        );
                    }

                notmux_git::repository::create_worktree_with_start_point(
                    &git_root_clone,
                    &branch_clone,
                    &target,
    ...
    pub fn show_color_picker(
        ...
        self.request_broker.update(cx, |broker, cx| {
            broker.push_overlay_request(
                notmux_workspace::requests::OverlayRequest::ColorPicker {
                    project_id,
                    position,
                },
                cx,
    ...
    pub fn show_folder_color_picker(
        ...
        self.request_broker.update(cx, |broker, cx| {
            broker.push_overlay_request(
                notmux_workspace::requests::OverlayRequest::FolderColorPicker {
                    folder_id,
                    position,
                },
                cx,
    ...
    pub fn sync_remote_color(
        ...
        {
            let server_id = notmux_core::client::strip_prefix(project_id, &conn_id);
            (send_action)(
                &conn_id,
    ...
    pub(crate) fn request_context_menu(
        ...
        self.request_broker.update(cx, |broker, cx| {
            broker.push_overlay_request(
                notmux_workspace::requests::OverlayRequest::ContextMenu {
                    project_id,
                    position,
                },
                cx,
    ...
    fn push_group_cursor_items(
        &self,
        project_id: &str,
        layout: &Option<notmux_workspace::state::LayoutNode>,
        service_names: &HashMap<String, Vec<String>>,
        hook_terminal_ids: &HashMap<String, Vec<String>>,
        cursor_items: &mut Vec<SidebarCursorItem>,
    ) {
                ...
                if let Some(ref sm) = self.service_manager {
                    sm.update(cx, |sm, cx| {
                        ...
                        if let Some(inst) = sm.instances().get(&key) {
                            match inst.status {
                                notmux_services::manager::ServiceStatus::Running
                                | notmux_services::manager::ServiceStatus::Starting => {
...
pub struct SidebarServiceInfo {
    pub name: String,
    pub status: notmux_services::manager::ServiceStatus,
    pub ports: Vec<u16>,
...
pub struct SidebarHookInfo {
    ...
    pub label: String,
    pub status: notmux_workspace::state::HookTerminalStatus,
    pub command: String,
    pub cwd: String,
}
                    ...
                    .map(|p| {
                        let services = sm
                            ...
                            .map(|inst| SidebarServiceInfo {
                                ...
                                is_docker: matches!(
                                    inst.kind,
                                    notmux_services::manager::ServiceKind::DockerCompose { .. }
                                ),
                            })
                            .collect();
                        (p.id.clone(), services)
            ...
            if !snapshot.services.is_empty() && !project_services.contains_key(&project.id) {
                ...
                let services = snapshot
                    ...
                    .map(|api_svc| SidebarServiceInfo {
                        ...
                        status: notmux_services::manager::ServiceStatus::from_api(
                            &api_svc.status,
                            api_svc.exit_code,
                        ),
...
impl Sidebar {
    ...
    fn render_files_view(&mut self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let sidebar_settings = self.sidebar_settings(cx);
        fe.update(cx, |fe, cx| {
            fe.set_monochrome_icons(sidebar_settings.monochrome_icons, cx);
            fe.set_show_hidden(sidebar_settings.show_hidden, cx);
        });
