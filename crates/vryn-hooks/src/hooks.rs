...
impl HookRunner {
    ...
    fn create_hook_terminal(
        ...
        let terminal_id = if keep_alive {
            ...
            // matching __vryn_hook_exit:<code> and updates hook status.
            script.push_str("; __vryn_rc=$?; printf '\\033]0;__vryn_hook_exit:%d\\007' \"$__vryn_rc\"; exec \"${SHELL:-sh}\"");
...
/// Includes base project vars and, for worktree projects, VRYN_BRANCH.
pub fn terminal_hook_env(
    ...
    if is_worktree {
        ...
        let branch = vryn_git::get_git_status(path)
            .and_then(|s| s.branch)
            .or_else(|| vryn_git::get_current_branch(path));
        if let Some(branch) = branch {
            env.insert("VRYN_BRANCH".into(), branch);
        }
    }
    env
}
...
fn build_headless_command(command: &str, env_vars: &HashMap<String, String>) -> std::process::Command {
    #[cfg(unix)]
    let mut cmd = vryn_core::process::command("sh");
    #[cfg(unix)]
    cmd.arg("-c").arg(command);

    #[cfg(windows)]
    let mut cmd = vryn_core::process::command("cmd");
    #[cfg(windows)]
    cmd.arg("/C").arg(command);
    ...
    if let Some(path) = env_vars.get("VRYN_PROJECT_PATH") {
...
fn build_hook_label(hook_type: &str, env_vars: &HashMap<String, String>, project_name: &str) -> String {
    let context = env_vars.get("VRYN_BRANCH")
        .map(|s| s.as_str())
...
fn run_hook(
    ...
    if let Some(runner) = runner {
        let project_path = env_vars.get("VRYN_PROJECT_PATH").cloned().unwrap_or_default();
...
fn project_env(
    ...
    env.insert("VRYN_PROJECT_ID".into(), project_id.into());
    env.insert("VRYN_PROJECT_NAME".into(), project_name.into());
    env.insert("VRYN_PROJECT_PATH".into(), project_path.into());
    if let Some(id) = folder_id {
        env.insert("VRYN_FOLDER_ID".into(), id.into());
    }
    if let Some(name) = folder_name {
        env.insert("VRYN_FOLDER_NAME".into(), name.into());
    }
    env
}
...
pub fn fire_on_worktree_create(
    ...
    if let Some(cmd) = resolve_hook(project_hooks, global_hooks, |h| &h.worktree.on_create) {
        let mut env = project_env(project_id, project_name, project_path, folder_id, folder_name);
        env.insert("VRYN_BRANCH".into(), branch.into());
...
fn merge_env(
    ...
    let mut env = project_env(project_id, project_name, project_path, folder_id, folder_name);
    env.insert("VRYN_BRANCH".into(), branch.into());
    env.insert("VRYN_TARGET_BRANCH".into(), target_branch.into());
    env.insert("VRYN_MAIN_REPO_PATH".into(), main_repo_path.into());
    env
}
...
pub fn fire_on_rebase_conflict(
    ...
    if let Some(cmd) = resolve_hook(project_hooks, global_hooks, |h| &h.worktree.on_rebase_conflict) {
        ...
        env.insert("VRYN_REBASE_ERROR".into(), rebase_error.into());
...
pub fn fire_terminal_on_close(
    ...
    if let Some(cmd) = resolve_hook_with_parent(project_hooks, parent_hooks, global_hooks, |h| &h.terminal.on_close) {
        let mut env = project_env(project_id, project_name, project_path, folder_id, folder_name);
        env.insert("VRYN_TERMINAL_ID".into(), terminal_id.into());
        if let Some(name) = terminal_name {
            env.insert("VRYN_TERMINAL_NAME".into(), name.into());
        }
        if let Some(code) = exit_code {
            env.insert("VRYN_EXIT_CODE".into(), code.to_string());
        }
        if is_worktree {
            let path = std::path::Path::new(project_path);
            let branch = vryn_git::get_git_status(path)
...
mod tests {
    ...
    fn build_hook_label_uses_branch() {
        let mut env = HashMap::new();
        env.insert("VRYN_BRANCH".into(), "feature/foo".into());
    ...
    #[test]
    fn valid_env_keys() {
        assert!(is_valid_env_key("VRYN_PROJECT_PATH"));
    ...
    fn apply_on_create_with_env_vars() {
        ...
        let mut env = HashMap::new();
        env.insert("VRYN_PROJECT_ID".into(), "proj-123".into());
        let result = apply_on_create(&shell, "echo hello", &env);
        match &result {
            ShellType::Custom { path: _, args } => {
                let cmd = &args[1];
                assert!(cmd.contains("export VRYN_PROJECT_ID="), "got: {}", cmd);
    ...
    fn apply_shell_wrapper_with_env_vars() {
        ...
        let mut env = HashMap::new();
        env.insert("VRYN_PROJECT_NAME".into(), "my-project".into());
        let result = apply_shell_wrapper(&shell, "wrapper {shell}", &env);
        match &result {
            ShellType::Custom { path: _, args } => {
                let cmd = &args[1];
                assert!(cmd.contains("export VRYN_PROJECT_NAME="), "got: {}", cmd);
