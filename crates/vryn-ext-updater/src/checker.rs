...
fn check_blocking(app_version: &str) -> Result<Option<ReleaseAsset>> {
    let client = reqwest::blocking::Client::builder()
        ...
        .user_agent(format!("vryn/{}", app_version))
        .build()
        .context("failed to build HTTP client")?;

    let http_resp = client
        .get("https://api.github.com/repos/contember/vryn/releases/latest")
        .send()
...
fn platform_asset_name() -> &'static str {
    ...
    return "vryn-linux-x64.tar.gz";
    #[cfg(all(target_os = "linux", target_arch = "aarch64"))]
    return "vryn-linux-arm64.tar.gz";
    #[cfg(all(target_os = "macos", target_arch = "aarch64"))]
    return "vryn-macos-arm64.zip";
    #[cfg(all(target_os = "macos", target_arch = "x86_64"))]
    return "vryn-macos-x64.zip";
    #[cfg(all(target_os = "windows", target_arch = "x86_64"))]
    return "vryn-windows-x64.zip";
    #[cfg(all(target_os = "windows", target_arch = "aarch64"))]
    return "vryn-windows-arm64.zip";
