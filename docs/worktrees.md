...
NotMux has built-in support for git worktrees, letting you work on multiple branches of a repository simultaneously without switching branches or cloning the repo again. Each worktree gets its own project in the sidebar with independent terminal sessions.

## Overview

A git worktree is a linked checkout of a branch in a separate directory, sharing the same `.git` history as the main repo. NotMux treats each worktree as a child project of the repository it was created from. When you create a worktree, NotMux:
...
NotMux runs a background watcher that polls every **30 seconds** to:

- **Discover new worktrees** -- If you create a worktree from the command line (outside NotMux), the watcher detects it via `git worktree list` and adds it to the sidebar automatically.
...
NotMux handles monorepos where the project directory is a subdirectory of the git repository root. When creating a worktree from a monorepo project:

- NotMux detects the git root via `git rev-parse --show-toplevel`
...
Worktree settings live in `~/.config/notmux/settings.json` under the `worktree` key:
