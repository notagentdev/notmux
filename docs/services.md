...
NotMux can manage background services (dev servers, databases, watchers) alongside your terminals. Services are defined per-project in an `notmux.yaml` file and/or auto-detected from Docker Compose.

## Overview

There are two kinds of services:

- **NotMux services** -- shell commands that NotMux spawns and manages in PTY processes. Defined in `notmux.yaml`.
- **Docker Compose services** -- containers managed by Docker Compose. Auto-detected from compose files or configured in `notmux.yaml`.
...
## notmux.yaml Configuration

Place an `notmux.yaml` file in your project root. It has two top-level keys:
...
NotMux detects and integrates Docker Compose services automatically.

### Auto-Detection

When a project is opened, NotMux checks for compose files in this order:
...
If one is found and the `docker compose` CLI is available, NotMux lists the services defined in it. Services with `deploy.replicas: 0` are excluded.

### Configuration

Use the `docker_compose` section in `notmux.yaml` to customize behavior:
...
- **`file`** -- Path to the compose file, relative to the project root. If omitted, NotMux auto-detects.
...
Docker Compose integration works even without an `notmux.yaml` file -- NotMux will auto-detect compose files in any project.

### Status Polling

NotMux polls Docker service statuses every 5 seconds using `docker compose ps`. This updates each service's status and detected ports in the sidebar without manual refresh.
...
When `restart_on_crash: true` is set for an NotMux service:

1. The service exits with a non-zero code.
2. The old terminal is cleaned up.
3. The status changes to **Restarting**.
4. After `restart_delay_ms` milliseconds, NotMux spawns a new process.
...
NotMux services can reconnect to existing sessions across app restarts (when using a session backend like tmux). The terminal ID for each service is persisted in the workspace file. Docker log viewer PTYs are ephemeral and not persisted.
...
- **Reload** -- Re-read `notmux.yaml` and update services. New services are added, removed services are stopped, and unchanged running services keep running.
...
Clicking a running NotMux service shows its terminal output. Clicking a Docker service opens its log viewer.

## Port Detection

NotMux automatically detects TCP ports that a running service is listening on.

### How It Works

1. After a service starts, NotMux waits 2 seconds for the process to bind its port.
...
This starts the Vite dev server as an NotMux service and monitors `postgres` and `redis` containers from Docker Compose. Any other services in the compose file will appear in a separate "Other" group.
...
### Docker Compose Only (No notmux.yaml Needed)

If your project has a `docker-compose.yml` and no `notmux.yaml`, NotMux will still auto-detect the compose file and show all services in the sidebar. No configuration needed.

To customize which Docker services are highlighted or to use a non-standard compose file path, add an `notmux.yaml` with just the `docker_compose` section:
